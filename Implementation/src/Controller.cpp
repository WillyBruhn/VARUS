...
//		if(1 == param->lessInfo){
//			exportTotalObservationCSVlessInfo("");
//		}else{
//			exportTotalObservationCSV("");
//		}
	...
	if(param->estimator == 0){
		...
		if(downloadableRuns.size() > 0){
			...
			for(unsigned int i = 1; i < downloadableRuns.size(); i++){
				...
				// score is equal
			else if(maxProfit == downloadableRuns[i]->expectedProfit){
					candidates.push_back(downloadableRuns[i]);
				}
			}
...
void Controller::profit(Run *d) {
	...
	// calculate the contribution of all entries with no observations
//	pr += (score(d->getPNoObs()*param->batchSize + 0) - score(0))*(param->numOfBlocks - d->p.size());

//	pv+= d->getPNoObs()*(d->getSize() - d->getPSize());
//	DEBUG(1,"pv:" << pv);
//	DEBUG(1,"pr:" << pr);
//	DEBUG(1,d->getSize());
//	DEBUG(1,d->getPSize());
//	DEBUG(1,d->getObservationSize());
...
void Controller::exportTotalObservationCSV(string name) {
	...
	if(param->exportObservationsToFile != 0)
	{
		...
		for(unsigned int k = 0; k < runs.size(); k++) {
			f << ";" << runs[k]->accesionId << ".obs;" << runs[k]->accesionId<< ".p;" << runs[k]->accesionId<< ".score";
		}
		f << endl;

		if(param->simulation != 1){
			for(unsigned int j=0; j < totalObservations.size(); j++) {
				f << chrom->translate2str[j]<< ";" << totalObservations[j] << ";" << totalScore << ";" << totalProfit << ";" << name;

				for(unsigned int k = 0; k < runs.size(); k++) {
					f << ";" << runs[k]->observations[j] << ";" << runs[k]->p[j] << ";" << runs[k]->expectedProfit;
				}
				f << endl;
			}
		}else{
			for(unsigned int j=0; j < totalObservations.size(); j++) {
				f << sim->translate2str[j]<< ";" << totalObservations[j] << ";" << totalScore << ";" << totalProfit << ";" << name;
...
void Controller::createDiceFromRuns() {
	 ...
	 * allready creadted as a die are listed. Then a "Runlist.txt" is read in.
...
void Controller::updateDownloadableRuns(){
	/*! \brief Checks if all reads from a run are downloaded, and deletes it from
	 * downloadalbeRuns if it is the case.
	 *
	 * Also checks if the quality of a run is too bad and hence should not be downloaded from anymore.
	 *
	 * Checks all runs. Should be improved by only checking the run that was downloaded instead
	 * of checking all runs.
	 */

//	for(unsigned int i = 0; i < downloadableRuns.size(); i++){
//		if(downloadableRuns[i]->badQuality == true){
//			DEBUG(0,"Deleting run " << downloadableRuns[i]->accesionId << " because of bad quality.");
//			//downloadableRuns.erase(downloadableRuns.begin()+i);
////			downloadableRuns.erase(std::remove_if(downloadableRuns.begin(), downloadableRuns.end(),
////			                       [](Run *r) { return r->badQuality; }), downloadableRuns.end());
//		}
//		else if(downloadableRuns[i]->timesDownloaded*param->batchSize >= downloadableRuns[i]->numOfSpots){
//			DEBUG(0,"Deleting run " << downloadableRuns[i]->accesionId << " because no reads are left to download.");
//			//downloadableRuns.erase(downloadableRuns.begin()+i);
////			downloadableRuns.erase(std::remove_if(downloadableRuns.begin(), downloadableRuns.end(),
////			                       [](Run *r) { return r->badQuality; }), downloadableRuns.end());
//		}
